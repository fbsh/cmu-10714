...
class Tensor(Value):
    ...
    def spmm(self, sparse_matrix):
        return needle.ops.SparseMatrixMultiply(sparse_matrix)(self)
...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    for node in reverse_topo_order:
      node_list = node_to_output_grads_list[node]
      # vi = node_list[0]
      # if len(node_list) <= 1:
      #   if isinstance(vi, array_api.float64):
      #     vi = array_api.float32(vi)
      #   elif not vi.dtype == array_api.float32:
      #     vi.cached_data = array_api.float32(vi.realize_cached_data())
      # else:
      #   vi = sum_node_list(node_list)
      vi = sum_node_list(node_list)
      ...
      if len(node.inputs) == 0:
        continue
      gradients = node.op.gradient_as_tuple(vi, node)
      idx = 0
      for k in node.inputs:
        node_to_output_grads_list[k] = node_to_output_grads_list.get(k, list())
        node_to_output_grads_list[k].append(gradients[idx])
        idx += 1
    ### END YOUR SOLUTION
