...
def xavier_uniform(fan_in, fan_out, shape=None, gain=1.0, **kwargs):
    ...
    return rand(int(fan_in), int(fan_out), low=-a, high=a, **kwargs)
    ### END YOUR SOLUTION


def xavier_normal(fan_in, fan_out, shape=None, gain=1.0, **kwargs):
    ### BEGIN YOUR SOLUTION
    a = (gain) * math.sqrt(2.0/float(fan_in + fan_out))
    return randn(int(fan_in), int(fan_out), mean=0, std=a, **kwargs)
    ### END YOUR SOLUTION
...
def kaiming_uniform(fan_in, fan_out, shape=None, nonlinearity="relu", **kwargs):
    ...
    ### BEGIN YOUR SOLUTION
    a = math.sqrt(6.0/float(fan_in))
    if shape==None:
        shape = (int(fan_in), int(fan_out))
...
def kaiming_normal(fan_in, fan_out, shape=None, nonlinearity="relu", **kwargs):
    assert nonlinearity == "relu", "Only relu supported currently"
    ### BEGIN YOUR SOLUTION
    a = math.sqrt(2.0 / float(fan_in))
    if shape==None:
        shape = (int(fan_in), int(fan_out))
    return randn(*shape, mean=0, std=a, **kwargs)
    ### END YOUR SOLUTION
